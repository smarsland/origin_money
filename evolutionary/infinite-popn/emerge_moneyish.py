"""
This is jsut a copy of emerge-any, cut down to only do limited 
part of the h,s space.
"""
...
for dh00 in just2:  # note h can't go down from zero so -1 is left out.
    for dsSelf00 in just2:
        for dsOther00 in all3:                                    
...
#            print('{0} self-helps {1:.3f} fitness {2:.3f}\n'.format(commonStrat.name, helpedRate_common, fitness_common))
            ESSs[commonStrat.name] = [helpingRate_common, fitness_common, np.dot(w_common,range(args.nsl))]
        else: 
...
for strat in ESSs.keys():
    fp.write('{0} \t self-helps at rate {1:.3f} \t fitness {2:.3f} \t mean s {3:.3f}\n'.format(strat, ESSs[strat][0], ESSs[strat][1], ESSs[strat][2]))
