...
bbname = "CmeetsN:000000++ NmeetsC:000000--" 

BigB = name2bigbrother(bbname)

# print out the name of the BigB
@printf("# BigB: %s\n", bigbrother2name(BigB))

REPUTATION=:tokens #,:tokens,:scores] # binary, tokens, scores
WSTART = [.5,.0,.5]
noiseval = 0.1
...
w, fit, trans, totalscore = calc_stable_w_1strategies(X, BigB; w_init=WSTART, REPUTATION=REPUTATION, FORCE_UNTIL_MAXN=false, NOISE_VAL=noiseval)
...
for Y in values(strats)
    ...
    =#    
    resX, resY = calc_stable_w_2strategies(X, Y, BigB; rhoA=1.0, wA_init=WSTART, wB_init=WSTART, REPUTATION=REPUTATION, FORCE_UNTIL_MAXN=false, NOISE_VAL=noiseval)
    #println(keys(resY))
